        ...
        public void It_throws_sqlite_locked()
        {
            using (var connection = CreateConnection())
            {
                ...
                using (var reader = selectCommand.ExecuteReader())
                {
                    ...
                    var message = NativeMethods.sqlite3_errstr(Constants.SQLITE_LOCKED);
                    Assert.Equal(Strings.FormatSqliteNativeError(Constants.SQLITE_LOCKED, message), ex.Message);
                }
            ...
            using (var connection = CreateConnection())
            {
                ...
                using (var connection2 = CreateConnection())
                {
                    ...
                    using (var reader = selectCommand.ExecuteReader())
                    {
                        ...
                        Assert.Equal(Strings.FormatSqliteNativeError(Constants.SQLITE_BUSY, message), ex.Message);
                    }
